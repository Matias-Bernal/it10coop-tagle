...
public class ControlPedido extends UnicastRemoteObject implements IControlPedido {
	...
	public void modificarPedido(Long id, PedidoDTO modificado) throws Exception {
		...
		try {
			...
			pedido.setEstado_pedido (estado_PedidoAssemb.getEstado_Pedido(modificado.getEstado_pedido()));
			pedido.setFecha_recepcion_fabrica(modificado.getFecha_recepcion_fabrica());
			pedido.setFecha_solicitud_fabrica(modificado.getFecha_solicitud_fabrica());
			pedido.setFecha_solicitud_pedido(modificado.getFecha_solicitud_pedido());
			pedido.setNumero_pedido(modificado.getNumero_pedido());
			pedido.setPnc(modificado.getPnc());
...
<<<<<<< .mine
	public Vector<PedidoDTO> obtenerPedidos() throws Exception {
		...
		try {
			...
			Vector<Pedido> pedidos = (Vector<Pedido>) accesoBD.buscarPorFiltro(Pedido.class, "");
			for (int i = 0; i < pedidos.size(); i++) {
				PedidoDTO pedidoDTO = new PedidoDTO();
				
				pedidoDTO.setId(pedidos.elementAt(i).getId());
				Estado_PedidoAssembler estado_PedidoAssemb = new Estado_PedidoAssembler(accesoBD);
				pedidoDTO.setEstado_pedido (estado_PedidoAssemb.getEstado_PedidoDTO(pedidos.elementAt(i).getEstado_pedido()));
				pedidoDTO.setFecha_recepcion_fabrica(pedidos.elementAt(i).getFecha_recepcion_fabrica());
				pedidoDTO.setFecha_solicitud_fabrica(pedidos.elementAt(i).getFecha_solicitud_fabrica());
				...
				pedidoDTO.setPnc(pedidos.elementAt(i).getPnc());
				ReclamoAssembler reclamoAssemb = new ReclamoAssembler(accesoBD);
...
>>>>>>> .r18
	...
	@Override
	public Vector<PedidoDTO> obtenerPedidos_Fecha_Solicitud_Fabrica(Date fecha_solicitud_fabrica) throws Exception {
		AccesoBD accesoBD = new AccesoBD();
		Vector<PedidoDTO> pedidosDTO = new Vector<PedidoDTO>();
		try {
			accesoBD.iniciarTransaccion();
			Collection movCol =  accesoBD.obtenerObjetosFecha(Pedido.class, fecha_solicitud_fabrica.getYear(),fecha_solicitud_fabrica.getMonth(), fecha_solicitud_fabrica.getDay());
			PedidoAssembler pedidoAssem = new PedidoAssembler(accesoBD);
			for (int i = 0; i < movCol.size(); i++) {
				pedidosDTO.add(pedidoAssem.getPedidoDTO((Pedido)(movCol.toArray())[i]));
			}
	...
	public Vector<PedidoDTO> obtenerPedidos_Fecha_Recepcion_Fabrica(Date fecha_recepcion_fabrica) throws Exception {
		...
		try {
			accesoBD.iniciarTransaccion();
			Collection movCol =  accesoBD.obtenerObjetosFecha(Pedido.class, fecha_recepcion_fabrica.getYear(),fecha_recepcion_fabrica.getMonth(), fecha_recepcion_fabrica.getDay());
			PedidoAssembler pedidoAssem = new PedidoAssembler(accesoBD);
			for (int i = 0; i < movCol.size(); i++) {
				pedidosDTO.add(pedidoAssem.getPedidoDTO((Pedido)(movCol.toArray())[i]));
			}
